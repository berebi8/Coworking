/*
  # Fix Termination Notices Schema

  1. Changes
    - Remove 'completed' status from enum
    - Add unique constraint for active/draft notices
    - Add proper foreign key relationships
    - Add date validation
    - Create view with additional fields
...
-- Create partial unique index to ensure only one active or draft notice per company
...
SELECT 
  ...
  -- Add a check for existing active or draft notice
  EXISTS (
    SELECT 1 
    FROM termination_notices tn2 
    WHERE tn2.company_id = tn.company_id 
    AND tn2.status IN ('active', 'draft')
    AND tn2.id != tn.id
