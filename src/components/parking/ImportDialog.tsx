...
// Map of display types to internal types
const TYPE_MAP: { [key: string]: string } = {
  'Reserved': 'parking_reserved',
  'Unassigned': 'parking_unassigned',
  'EV Charging': 'parking_ev',
  'VIP': 'parking_vip'
};
...
export function ImportDialog({ isOpen, onClose, onComplete, locations }: ImportDialogProps) {
  ...
  const generateTemplate = () => {
    ...
    const sampleData = [
      {
        ...
        'Default Price': 500,
        'Quantity': 20,
        'Notes': 'Reserved parking space with 24/7 access'
      },
      {
        'Location': 'All Locations',
        'Type': 'Unassigned',
        'Name': 'Unassigned Parking Space',
        'Default Price': 300,
        'Quantity': 50,
        'Notes': 'First come, first served parking'
      },
      {
        'Location': locations[0]?.name || 'Main Office',
        'Type': 'EV Charging',
        ...
        'Default Price': 800,
        'Quantity': 10,
        'Notes': 'Electric vehicle charging station'
      }
    ];
    ...
    const validationNotes = [
      ...
      ['2. Type:'],
      ['   - Must be one of: Reserved, Unassigned, EV Charging, VIP'],
      ['3. Name:'],
      ['   - Must be unique for each parking type'],
      ['   - Cannot be empty'],
      ['4. Default Price:'],
    ...
    // Download the file
    XLSX.writeFile(wb, 'parking_import_template.xlsx');
  };
  ...
  const validateData = async (data: any[]): Promise<ValidationError[]> => {
    ...
    const { data: existingServices } = await supabase
      .from('addon_services')
      .select('name, type')
      .in('type', Object.values(TYPE_MAP))
      .neq('status', 'deleted');

    // Create a map of existing name+type combinations
    const existingCombos = new Map<string, boolean>();
    existingServices?.forEach(service => {
      const key = `${service.name}:${service.type}`;
      existingCombos.set(key, true);
    });

    // Track combinations within the import data
    const importCombos = new Map<string, number>();
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // Check type
      if (!Object.keys(TYPE_MAP).includes(row['Type'])) {
        errors.push({
          row: rowNum,
          field: 'Type',
          message: 'Must be one of: Reserved, Unassigned, EV Charging, VIP'
        });
        continue; // Skip further validation for this row
      }

      const internalType = TYPE_MAP[row['Type']];
      ...
      if (!row['Name']) {
        ...
        const combo = `${row['Name']}:${internalType}`;
        ...
        if (existingCombos.has(combo)) {
          errors.push({
            row: rowNum,
            field: 'Name',
            message: `A parking service with name "${row['Name']}" and type "${row['Type']}" already exists`
          });
        }

        // Check for duplicates within import data
        if (importCombos.has(combo)) {
          errors.push({
            row: rowNum,
            field: 'Name',
            message: `Duplicate entry: A parking service with name "${row['Name']}" and type "${row['Type']}" appears multiple times in the import file (first appears in row ${importCombos.get(combo)})`
          });
        } else {
          importCombos.set(combo, rowNum);
        }
      }
              ...
              {preview.errors.length > 0 ? (
                ...
                <div className="space-y-4">
                  <div className="flex items-center space-x-2 text-teal">
                    ...
                    <span className="font-medium">Ready to import {preview.data.length} parking spaces</span>
                  </div>
